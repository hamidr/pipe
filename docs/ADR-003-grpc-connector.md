...
shipped phases validated that pipe's pull-based backpressure, bounded
...
natural fit for pipe. The Rust ecosystem has a mature gRPC stack in
...
2. **Validates new properties**: Unlike WebSocket, gRPC has typed
   ...
   and HTTP/2 flow control. These exercise pipe's type system and
   error handling in ways WebSocket did not.

3. **High user demand**: gRPC is ubiquitous in production microservices.
   A pipe-grpc crate makes pipe immediately useful for service mesh
...
4. **Validates codegen integration**: Users bring protobuf-generated
   types. pipe-grpc must be generic over these types, validating that
   pipe's trait bounds compose with external codegen -- a property
...
- 80% of implementation time goes to broker protocol, not pipe
  architecture validation
- If pipe-grpc reveals a composability flaw, better to find it in a
  1-week connector than a 3-week Kafka connector

### Codegen coupling

gRPC's reliance on protobuf codegen is the main design tension.
pipe-grpc cannot own the generated types -- users run `tonic-build`
...
// Now use pipe operators: backpressure, map, filter, retry, fan-out
events
...
- Pipe's pull-based model over HTTP/2 flow control (tonic/hyper manage
  the HTTP/2 window; pipe pulls from tonic's stream via bounded channel)
- Type composition: user-generated protobuf types flow through pipe's
...
**Deliverables**:
- `pipe-grpc` crate with `streaming` module
...
- **Server-side gRPC handlers**: Accepting incoming gRPC calls and
  routing them into pipe pipelines. This requires deep integration with
  tonic's service trait and Tower middleware. Deferred until client-side
  patterns are validated.
- **Reflection / dynamic dispatch**: pipe-grpc assumes compiled protobuf
  types. Dynamic message handling is out of scope.
- **Load balancing / service discovery**: Handled by tonic's Channel
  layer, not by pipe-grpc.
- **Interceptors / middleware**: Users compose tonic interceptors before
  handing streams to pipe-grpc. No duplication of tonic's middleware.

## Consequences

### Positive

- gRPC streaming maps naturally to pipe's existing patterns (source,
...
- HTTP/2 flow control provides transport-level backpressure that
  composes with pipe's channel-based backpressure
...
- tonic + prost add significant transitive dependencies (~50 crates).
  Mitigated by separate crate -- core pipe stays dependency-free.
- Users must set up protobuf codegen (build.rs, .proto files) before
  they can use pipe-grpc. This is inherent to gRPC, not a pipe-grpc
  design choice.
- API ergonomics depend on tonic's types (Channel, Streaming, Request).
  pipe-grpc is a thin adapter, not a full abstraction -- tonic version
  bumps may require pipe-grpc major version bumps.
...
- **tonic version churn**: tonic is pre-1.0 (currently 0.14). Breaking
  changes in tonic could force pipe-grpc updates. Mitigation: pin tonic
  minor version; keep the adapter layer thin so updates are mechanical.
- **HTTP/2 backpressure opacity**: pipe cannot directly observe or
  control HTTP/2 flow control windows. If tonic buffers aggressively
  internally, pipe's backpressure may not propagate to the server.
  Mitigation: measure in integration tests; if needed, add a bounded
  channel between tonic's stream and pipe's pull operator (same pattern
  as SSE).
- **Codegen ergonomics**: If the API requires too much boilerplate to
  convert between tonic types and pipe types, adoption will suffer.
